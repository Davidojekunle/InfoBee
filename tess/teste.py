...
class DataHiveApp(ctk.CTk):
    ...
    def update_subscription_info(self):
        ...
        self.frames = {}
        for F in (DashboardPage, MyFilesPage, AnalysisPage, VisualsPage, PaymentPage, ProfilePage):
...
class MyFilesPage(ctk.CTkFrame):
    ...
    def populate_file_list(self):
        ...
        try:
            # Fetch file list from API if not already cached
            if not self.file_list:
                self.file_list = self.api_client.get_file_list()

            # Calculate start and end indices for current page
            start = self.current_page * self.files_per_page
            end = start + self.files_per_page
            current_files = self.file_list[start:end]

            # Create a button for each file on the current page
            ...
            if self.current_page > 0:
                prev_button = ctk.CTkButton(self.scrollable_frame, text="Previous", command=self.prev_page)
                prev_button.grid(row=len(current_files)+1, column=0, pady=5, padx=10, sticky="w")

            if end < len(self.file_list):
                next_button = ctk.CTkButton(self.scrollable_frame, text="Next", command=self.next_page)
                next_button.grid(row=len(current_files)+1, column=0, pady=5, padx=10, sticky="e")
        ...
        try:
            ...
            for i, viz in enumerate(visualizations):
                ...
                if image_path and os.path.exists(image_path):
                    ...
                    image.thumbnail((400, 300))  # Adjust size as needed
